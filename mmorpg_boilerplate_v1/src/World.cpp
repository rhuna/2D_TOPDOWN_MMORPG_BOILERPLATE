...
World::World() {
    ...
    message_ = "Walk with WASD. E talks or loots. Space attacks.";
...
void World::Draw() const {
    ...
    for (const auto& npc : npcs_) {
        DrawRectangleV(npc.position, npc.size, npc.color);
    ...
    for (const auto& drop : drops_) {
        if (!drop.taken) {
            DrawCircle(static_cast<int>(drop.position.x) + 12, static_cast<int>(drop.position.y) + 12, 8.0f, SKYBLUE);
        }
    }

    DrawRectangleV(player_.position, player_.size, player_.color);
    DrawHud();
}
...
void World::DrawMap() const {
    for (int y = 0; y < static_cast<int>(map_.size()); ++y) {
        for (int x = 0; x < static_cast<int>(map_[y].size()); ++x) {
            Rectangle tile{static_cast<float>(x * tileSize_), static_cast<float>(y * tileSize_), static_cast<float>(tileSize_), static_cast<float>(tileSize_)};
            bool isWall = map_[y][x] == '#';
            DrawRectangleRec(tile, isWall ? DARKGRAY : Color{30, 80, 35, 255});
            DrawRectangleLinesEx(tile, 1.0f, Fade(BLACK, 0.2f));
        }
    }
}
...
void World::DrawHud() const {
    ...
    DrawText(stats.str().c_str(), 16, 652, 20, WHITE);
