...
World::World() {
    ...
    camera_.zoom = 2.0f;
    message_ = "Large world loaded. Camera follows player. WASD moves, E interacts, Space attacks.";
    ...
    for (int y = 0; y < static_cast<int>(map_.size()); ++y) {
        for (int x = 0; x < static_cast<int>(map_[y].size()); ++x) {
            ...
            if (map_[y][x] == 'P') {
                ...
                elder.idleText = map_[y][x] == 'E' ? "The road is safer than it used to be." : "The world keeps going far beyond this road.";
...
void World::DrawMinimap() const {
    ...
    DrawText("World Map", originX + 6, originY + miniH + 6, 18, WHITE);
}
