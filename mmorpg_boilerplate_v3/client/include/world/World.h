...
public:
    ...
    // Replace the current snapshot of remote multiplayer actors.
    // localPlayerId is used so the client can avoid drawing its own state twice.
    void SetRemotePlayers(const std::unordered_map<int, Vector2>& remotePlayers, int localPlayerId);
...
private:
    ...
    // Snapshot of other networked players to render on top of the world.
