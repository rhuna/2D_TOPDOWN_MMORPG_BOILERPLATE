...
// Replace the latest remote-player snapshot provided by the networking layer.
void World::SetRemotePlayers(const std::unordered_map<int, Vector2>& remotePlayers, int localPlayerId) {
...
// Draw simple colored markers for remote networked players.
// This intentionally uses rectangles instead of full atlas animation so the
// multiplayer upgrade stays easy to understand and merge.
void World::DrawRemotePlayers() const {
    for (const auto& [id, position] : remotePlayers_) {
        // Skip drawing the local player twice once the server has assigned us an id.
        if (localPlayerId_ != 0 && id == localPlayerId_) {
            continue;
        }
        ...
        DrawRectangleRec(body, Color{60, 140, 255, 255});
        DrawRectangleLinesEx(body, 2.0f, WHITE);
        DrawText(TextFormat("P%d", id), static_cast<int>(position.x - 4.0f), static_cast<int>(position.y - 16.0f), 12, WHITE);
    }
}
