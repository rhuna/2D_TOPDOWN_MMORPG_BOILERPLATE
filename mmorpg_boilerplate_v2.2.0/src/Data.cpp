...
std::vector<std::string> BuildMap() {
    ...
    auto addLake = [&](int cx, int cy, int radiusX, int radiusY) {
        for (int y = cy - radiusY; y <= cy + radiusY; ++y) {
            for (int x = cx - radiusX; x <= cx + radiusX; ++x) {
                ...
                float nx = static_cast<float>(x - cx) / static_cast<float>(radiusX);
                float ny = static_cast<float>(y - cy) / static_cast<float>(radiusY);
                if (nx * nx + ny * ny <= 1.0f) {
                    setTile(x, y, '#');
                }
            }
    ...
    // Scatter slimes near roads and zones.
    ...
    // Clear roads if something blocked them.
    carveH(6, width - 8, 24, 2);
    carveH(10, width - 12, 72, 2);
    carveH(8, width - 16, 128, 2);
    carveH(12, width - 18, 184, 2);
    ...
    // Put village entities back after recarving.
    setTile(22, 22, 'P');
    setTile(28, 22, 'E');
    for (const auto& [x, y] : npcPositions) {
        setTile(x, y, 'N');
